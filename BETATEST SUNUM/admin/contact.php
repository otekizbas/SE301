...
?>
			...
			<?php
			include "db_baglan.php";
			$sorgu = mysql_query("SELECT * FROM  `contact`")
			?>
						<div id = "ordertable">
						<h4>Contact Table</h4>
	<table style="width:100%"; style ="border:1px">
  <tr>
  <td style="font-weight:bold">Name & Surname</td>
 ...
 <?php
		 if($sorgu){
						 while($duyuru = mysql_fetch_array($sorgu)){
								echo '<tr>';
										echo '<td align="left"; style="font-style: italic; color: #666">'.$duyuru['ad'].'</td>';
										echo '<td align="left"; style="font-style: italic; color: #666">'.$duyuru['emailadres'].'</td>';
										echo '<td align="left"; style="font-style: italic; color: #666">'.$duyuru['mesaj'].'</td>';
